...
| Fund | A one-page snapshot in three columns. Left, the record: the growth chart, point to point and median rolling returns against the index with the gap subtracted, and the shape of the book. Middle, the fund as an object: who runs it, size and cost, largest sectors, top holdings. Right, risk: return per unit of risk, the drawdown periods it has lived through, and how it behaves in a fall. Every card opens its full detail in a modal. |
