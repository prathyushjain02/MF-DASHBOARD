...
| Fund | A one-page snapshot in three columns. Left, the record: the growth chart, point to point and median rolling returns against the index with the alpha printed, and the shape of the book. Middle, the fund as an object: size and cost, largest sectors, top holdings. Right, risk and who is taking it: return per unit of risk, the drawdown periods it has lived through, how it behaves in a fall, and who runs it. Every card opens its full detail in a modal, and the header carries Add to compare and Add to portfolio. |
