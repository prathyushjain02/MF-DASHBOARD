...
| Portfolio builder | The same funds held together rather than read side by side. Weights in rupees or percent, opening on an equal split. One bold line with the holdings behind a switch, and beside it four tiles on the combined book: what it comes to (effective holdings, distinct names, top 10 weight), the cap mix read off the underlying stocks rather than the funds' categories, the eight largest sectors, and the twelve largest positions. Below, the metric table with the portfolio's own row and the overlap matrix read as duplication. |
