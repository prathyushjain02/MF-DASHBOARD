...
| Page | What it is |
|---|---|
| Equity overview | The page opens here. Five panels, one at a time: the four broad benchmarks side by side (an INR 100 race with a re-sorting leaderboard, the calendar years ranked, how far each index sits below its last high); the holding period tools (the Sensex through six crises, and every start year against every holding period); why equity and why India; how we choose and keep watching (the funnel, two anonymised managers to pick between, and the flags that trigger a relook); and the quarter's AWM House View. Live figures come from the nightly index build; the House View and the long Sensex tables are in data/overview.json and say so at the foot. |
| Category top funds | A tile per category. Click one for its shortlist as a table: returns at 1M, 3M, 6M, YTD, 1Y, 3Y and 5Y, median rolling 3Y and 5Y, AUM and up to three managers, longest serving first. Click a fund name for its own page. |
| All funds | Every scheme. Filter by search, category, AMC, band, minimum AUM, maximum downside capture, holdings and rated-only; sort on any column. Columns come in bands behind checkboxes: returns, rolling returns, risk metrics, capture ratios, market cap breakdown and size and cost. The headers and the scheme name are frozen, so the sheet scrolls under them. Click a row for the fund page, or tick it to carry into Compare or the portfolio builder. |
| Fund | A one-page snapshot in three columns. Left, the record: the growth chart, point to point and median rolling returns against the index with the alpha printed, and the shape of the book. Middle, the fund as an object: size and cost, largest sectors, top holdings. Right, risk and who is taking it: return per unit of risk, the drawdown periods it has lived through, how it behaves in a fall, and who runs it. Every card opens its full detail in a modal, and the header carries Select this fund, the same tick box the lists use and the same bar at the foot. |
...
| Sectors | Pick a sector and see who holds it, most exposed first and sortable on any column, with bounds to narrow the range: every fund in IT, or only the ones under 10%. Exposure is a share of the fund's own equity book. Tick rows to carry them into Compare or the portfolio builder. |
